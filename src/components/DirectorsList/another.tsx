...
export default function DirectorsList({ directors }: Props) {
  ...
  const handleClickDirector = (name: string, index: number) => {
    ...
    if (newValue) {
      ...
      // CALCOLO ALTEZZA CREATIVEABOUT
      setTimeout(() => {
        const el = document.querySelector<HTMLDivElement>(
          `.questoMobile .creativeAbout[data-name="${newValue}"]`
        );
        if (el) {
          setCreativeHeights((prev) => ({ ...prev, [newValue]: el.offsetHeight }));
          console.log("Altezza di", newValue, "=", el.offsetHeight);
        }
      }, 50); // timeout breve per assicurarsi che il div sia renderizzato
    } else {
    ...
    <>
      <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
        {sortedDirectors.map((director, i) => {
          ...
          return (
            <div key={director.name} ref={(el) => {
  sectionRefs.current[director.name] = el; // <-- solo assegnamento
}}>
              <h2
              ...
              <div
                ...
                style={{
                  maxHeight: isOpen
                    ? creativeHeights[director.name]
                      ? `${creativeHeights[director.name] + window.innerHeight * 0.28 + 30}px`
                      : "90vh"
                    : "0px",
                  overflow: "hidden",
              ...
              >
                <div
                  ...
                  style={{ height: "90vh", overflowY: "hidden", cursor: isDragging ? "grabbing" : "grab", display: "block" }}
                >
                  {/* About */}
                  <div className="creativeAbout" data-name={director.name}>
                    ...
                    {director.projects?.map((project, index) => (
                      ...
                      >
                        {project.thumbnail?.url && (
                          <img style={{ maxHeight: "25vh" }} className="projectThumbnail" src={project.thumbnail.url} alt={project.title} loading="eager" />
                        )}
